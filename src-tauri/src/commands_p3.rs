// ============================================================
// SynthesisOverthrust — src-tauri/src/commands_p3.rs
...
pub async fn sync_write_note(path: String, content: String, force: Option<bool>) -> Result<Value> {
    post(
        "/sync/write",
        json!({"path":path,"content":content,"force":force.unwrap_or(false)}),
    )
    .await
    .map_err(|e| NfError::Sidecar(e.to_string()))
}
...
pub async fn sync_resolve(note_path: String, resolution: String) -> Result<Value> {
    post(
        "/sync/resolve",
        json!({"note_path":note_path,"resolution":resolution}),
    )
    .await
    .map_err(|e| NfError::Sidecar(e.to_string()))
}
...
pub async fn collab_delete_room(room_id: String) -> Result<Value> {
    ...
    reqwest::Client::new()
        ...
        .send()
        .await
        .map_err(|_| NfError::SidecarUnavailable)?
        .json::<Value>()
        .await
        .map_err(|e| NfError::Sidecar(e.to_string()))
}
...
pub async fn plugins_fire_hook(hook: String, payload: Option<Value>) -> Result<Value> {
    post(
        "/plugins/fire",
        json!({"hook":hook,"payload":payload.unwrap_or(json!({}))}),
    )
    .await
    .map_err(|e| NfError::Sidecar(e.to_string()))
}
...
pub async fn mobile_generate_key(label: Option<String>) -> Result<Value> {
    post(
        "/mobile/keys/generate",
        json!({"label":label.unwrap_or_else(||"Mobile companion".into()),"user_id":"default"}),
    )
    .await
    .map_err(|e| NfError::Sidecar(e.to_string()))
}
