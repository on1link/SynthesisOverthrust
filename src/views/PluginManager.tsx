// ============================================================
// SynthesisOverthrust — src/PluginManager.tsx
...
const MOCK_PLUGINS = [
  { id: "anki-export", name: "Anki Export", version: "1.2.0", enabled: true, hooks: ["on_sr_review"], description: "Exports SR cards to Anki-compatible .apkg files for offline study.", author: "on1link" },
  { id: "xp-webhook", name: "XP Webhook", version: "0.8.1", enabled: false, hooks: ["on_xp_gain", "on_level_up"], description: "Posts XP events to a Discord/Slack webhook for team accountability.", author: "on1link" },
  { id: "daily-digest", name: "Daily Digest", version: "1.0.0", enabled: true, hooks: ["on_daily_reset"], description: "Generates a daily Obsidian note summarising tasks done, XP gained, and SR performance.", author: "on1link" },
  { id: "pomodoro", name: "Pomodoro Timer", version: "0.5.0", enabled: false, hooks: ["on_grind_session_end"], description: "Auto-starts 25/5 pomodoro timers around grind sessions.", author: "on1link" },
  { id: "github-tracker", name: "GitHub Activity", version: "1.1.0", enabled: true, hooks: ["on_daily_reset", "on_task_complete"], description: "Fetches daily GitHub commit count and awards XP for code contributions.", author: "on1link" },
];
  ...
  return (
    <div style={col_()}>
      ...
      <div style={{ fontFamily: F.body, fontSize: 13, color: C.muted, lineHeight: 1.8 }}>
        Extend SynthesisOverthrust with hooks into every learning event. Plugins live in{" "}
        <code style={{ background: C.surface2, padding: "1px 6px", borderRadius: 4, color: C.accent, fontSize: 12 }}>
          ~/.local/share/synthesis-overthrust/plugins/
        </code>
      </div>
          ...
          <div style={{ flex: 1, ...col_(10) }}>
            {plugins.map(p => (
              <div key={p.id}
                ...
                style={{
                  ...card(p.enabled ? C.gold : C.muted), cursor: "pointer", transition: "all 0.18s",
                ...
                <div style={{ ...row(), justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
                  <div style={{ ...row(10) }}>
                    ...
                    <div
                      ...
                      style={{
                        width: 38, height: 20, borderRadius: 10, background: p.enabled ? C.gold : C.border,
                  ...
                  <div style={{ ...row(4), flexWrap: "wrap" }}>
                    {p.hooks.slice(0, 3).map(h => (
                      <span key={h} style={{
                        padding: "1px 6px", borderRadius: 4, background: `${HOOK_COL[h] || C.accent}18`,
      ...
      {tab === "create" && (
        <div style={col_(14)}>
          ...
          <div style={{ ...card(C.green), fontFamily: F.mono, fontSize: 12, lineHeight: 2.0, color: C.text2, whiteSpace: "pre", overflowX: "auto", padding: 20 }}>
            {`# ~/.local/share/synthesis-overthrust/plugins/my_plugin.py
